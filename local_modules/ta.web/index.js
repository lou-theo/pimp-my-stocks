...
async function kmeans(data, clusters) {
    var means = [],
        ...
        n,
        changed = false,
        init = Math.round(data.length / (clusters + 1));
...
async function normalize(data, marg = 0) {
    for (
        var i = 0,
            max = (await Math.max.apply(null, data.slice())) * (1 + marg),
            min = (await Math.min.apply(null, data.slice())) * (1 - marg),
            norm = [];
        i < data.length;
        i++
    )
        norm.push(1 - (max - data[i]) / (max - min));
    return norm;
}
async function denormalize(data, norm, marg = 0) {
    for (
        ...
        dnorm.push(min + norm[i] * (max - min));
    return dnorm;
}
async function normalize_pair(data1, data2) {
    for (
        var i = 1, f = (data1[0] + data2[0]) / 2, ret = [[f, f]];
        i < data1.length;
        i++
    )
        ret.push([
            ret[ret.length - 1][0] *
                ((data1[i] - data1[i - 1]) / data1[i - 1] + 1),
            ret[ret.length - 1][1] *
                ((data2[i] - data2[i - 1]) / data2[i - 1] + 1),
        ]);
    return ret;
}
async function normalize_from(data, value) {
    for (var i = 1, ret = [value]; i < data.length; i++)
        ret.push(
            ret[ret.length - 1] * ((data[i] - data[i - 1]) / data[i - 1] + 1)
        );
    return ret;
}
async function standardize(data) {
    var mean = await module.exports.sma(data, data.length),
        std = await module.exports.std(data),
        res = [];
...
async function ssd(data, length = data.length) {
    for (var i = length, sd = []; i <= data.length; i++) {
        var tmp = data.slice(i - length, i),
            mean = await module.exports.sma(tmp, length),
            sm = 0;
...
async function er(data) {
    var wins = [],
        ...
        lp = 1;
    for (let i in data)
        data[i] >= 0 ? wins.push(1 + data[i]) : losses.push(1 + data[i]);
    var win = wins.length / data.length,
        loss = losses.length / data.length;
    for (let i in losses) lp *= losses[i];
    for (let i in wins) wp *= wins[i];
    return (
        ((wp ** (1 / wins.length) - 1) * 100 * win +
            (lp ** (1 / losses.length) - 1) * 100 * loss) /
        100
    );
}
...
async function avgwin(data) {
    for (var i = 0, wins = []; i < data.length; i++)
        if (data[i] >= 0) wins.push(data[i]);
...
async function zscore(data, len = data.length) {
    for (
        var i = len - 1, out = [], pl = data.slice(0, len - 1);
        i < data.length;
        i++
    ) {
        pl.push(data[i]);
...
async function asi(data) {
    for (var i = 1, a = []; i < data.length; i++) {
        var c = data[i][1],
            ...
            l = data[i][2],
            ly = data[i - 1][2],
            k = hy - c > ly - c ? hy - c : ly - c,
            o = data[i][0],
            oy = data[i - 1][0],
            r,
            t =
                Math.max(data[i][0], data[i - 1][0]) -
                Math.min(data[i][2], data[i - 1][2]);
...
const fib = async (start, end) => [
    start,
    (end - start) * 0.236 + start,
    (end - start) * 0.382 + start,
    (end - start) * 0.5 + start,
    (end - start) * 0.618 + start,
    (end - start) * 0.786 + start,
    end,
    (end - start) * 1.618 + start,
    (end - start) * 2.618 + start,
    (end - start) * 3.618 + start,
    (end - start) * 4.236 + start,
];
async function pr(data, length = 14) {
    for (var i = length, n = []; i <= data.length; i++) {
        var pl = data.slice(i - length, i),
            highd = await Math.max.apply(null, pl),
            lowd = await Math.min.apply(null, pl);
...
async function lsma(data, length = 25) {
    for (var i = length, lr = []; i <= data.length; i++) {
        ...
        m =
            (sum_xy - (sum_x * sum_y) / length) /
            (sum_xx - (sum_x * sum_x) / length);
...
async function don(data, length = 20) {
    for (var i = length, channel = []; i <= data.length; i++) {
        ...
        var max = Math.max.apply(null, highs.slice()),
            min = Math.min.apply(null, lows.slice());
        channel.push([max, (max + min) / 2, min]);
    }
    return channel;
}
async function ichimoku(
    data,
    length1 = 9,
    length2 = 26,
    length3 = 52,
    ...
    for (var i = 0, pl = [], cloud = [], place = []; i < data.length; i++) {
        ...
        if (pl.length >= length3) {
            ...
            var tsen =
                    (Math.max.apply(
                        null,
                        highs.slice(highs.length - length1, highs.length)
                    ) +
                        Math.min.apply(
                            null,
                            lows.slice(lows.length - length1, lows.length)
                        )) /
                    2,
                ksen =
                    (Math.max.apply(
                        null,
                        highs.slice(highs.length - length2, highs.length)
                    ) +
                        Math.min.apply(
                            null,
                            lows.slice(lows.length - length2, lows.length)
                        )) /
                    2,
                senka = data[i][1] + ksen,
                senkb =
                    (Math.max.apply(
                        null,
                        highs.slice(highs.length - length3, highs.length)
                    ) +
                        Math.min.apply(
                            null,
                            lows.slice(lows.length - length2, lows.length)
...
async function stoch(data, length = 14, smoothd = 3, smoothk = 3) {
    ...
    for (
        var i = 0, stoch = [], high = [], low = [], ka = [];
        i < data.length;
        i++
    ) {
        high.push(data[i][0]), low.push(data[i][2]);
        if (high.length >= length) {
            var highd = await Math.max.apply(null, high),
                lowd = await Math.min.apply(null, low),
...
async function wsma(data, length = 14) {
    for (var i = length, wsm = [], weight = 1 / length; i <= data.length; i++) {
        if (wsm.length > 0) {
            wsm.push(
                (data[i - 1] - wsm[wsm.length - 1]) * weight +
                    wsm[wsm.length - 1]
            );
            continue;
        }
...
async function pwma(data, length = 14) {
    for (
        var i = length / 2, b = length, weight = 0, wmaa = [], weights = [];
        i >= 1;
        i--, b--
    ) {
        if (i % 1 !== 0) {
    ...
    for (var i = length; i <= data.length; i++) {
        var average = 0,
            pl = data.slice(i - length, i);
        for (var a = length - 1; a >= 0; a--)
            average += (pl[a] * weights[a]) / weight;
        wmaa.push(average);
    }
    return wmaa;
}
async function hwma(data, length = 14) {
    for (
        var i = 1, b = length, weight = 0, wmaa = [], weights = [];
        i <= length / 2;
        i++, b--
    ) {
        if (i % 1 !== 0) {
...
async function ema(data, length = 12) {
    for (
        ...
        if (ema.length > 0) {
            ema.push(
                (data[i - 1] - ema[ema.length - 1]) * weight +
                    ema[ema.length - 1]
            );
            continue;
        }
...
async function hull(data, length = 14) {
    var pl = [],
        hma = [],
        ewma = await module.exports.wma(data, length),
...
async function kama(data, len1 = 10, len2 = 2, len3 = 30) {
    for (
        var i = len1 + 1,
            ka = await module.exports.sma(data, len1),
            ka = [ka[ka.length - 1]];
        i < data.length;
        i++
    ) {
        var vola = 0,
            change = Math.abs(data[i] - data[i - len1]);
        for (var a = 1; a < len1; a++)
            vola += Math.abs(data[i - a] - data[i - a - 1]);
        var sc =
            ((change / vola) *
                (2 / (len2 + 1) - 2 / (len3 + 1) + 2 / (len3 + 1))) **
            2;
...
async function macd(data, length1 = 12, length2 = 26) {
    ...
    var ema = await module.exports.ema(data, length1),
        emb = await module.exports.ema(data, length2),
        macd = [];
...
async function bandwidth(data, length = 14, deviations = 1) {
    for (
        ...
        boll.push((band[i][0] - band[i][2]) / band[i][1]);
    return boll;
}
async function keltner(data, length = 14, devi = 1) {
    var closing = [],
        atr = await module.exports.atr(data, length),
        kma,
        kelt = [];
    for (let i in data)
        closing.push((data[i][0] + data[i][1] + data[i][2]) / 3);
...
async function std(data, length = data.length) {
    ...
    return Math.sqrt(
        data.reduce((sq, n) => sq + Math.pow(n - mean, 2), 0) / length
    );
}
    ...
    for (var i = 0, sd = []; i < sims; i++) {
        ...
        for (var x = 0; x < length; x++) {
            ...
            for (var y = 1; y < projected.length; y++) {
                var df = await module.exports.dif(
                    projected[y],
                    projected[y - 1]
                );
                change.push(df);
            ...
            var mean = await module.exports.sma(change, change.length),
                std = await module.exports.std(change),
                random = await module.exports.normsinv(Math.random());
            projected.push(
                projected[projected.length - 1] *
                    Math.exp(mean[0] - (std * std) / 2 + std * random)
            );
        }
        sd.push(projected);
    }
    ...
    for (var i = length, aroon = []; i <= data.length; i++) {
        ...
        aroon.push(
            (100 *
                (length -
                    1 -
                    pl.reverse().findIndex((x) => x === hl[length - 1]))) /
                (length - 1)
        );
    }
    return aroon;
...
async function aroon_down(data, length = 10) {
    for (var i = length, aroon = []; i <= data.length; i++) {
        ...
        hl.sort((a, b) => a - b);
        aroon.push(
            (100 * (length - 1 - pl.reverse().findIndex((x) => x === hl[0]))) /
                (length - 1)
        );
    }
    return aroon;
...
async function roc(data, length = 14) {
    for (var i = length, roc = []; i <= data.length; i++)
        roc.push((data[i - 1] - data[i - length]) / data[i - length]);
    return roc;
}
async function cop(data, len1 = 11, len2 = 14, len3 = 10) {
    for (
        var max = Math.max(len1, len2), co = [], i = max + len3;
        i < data.length;
        i++
    ) {
        var r1 = data.slice(i - (max + len3), i),
            r2 = r1.slice(),
            tmp = [];
        (r1 = await module.exports.roc(r1, len1)),
            (r2 = await module.exports.roc(r2, len2)),
            r1.splice(0, r1.length - r2.length),
            r2.splice(0, r2.length - r1.length);
...
async function kst(
    ...
    for (
        var ks = [],
            fs = [],
            ms = Math.max(r1, r2, r3, r4) + Math.max(s1, s2, s3, s4),
            i = ms;
        i <= data.length;
        i++
    ) {
        ...
        ks.push(
            rcma1[rcma1.length - 1] +
                rcma2[rcma2.length - 1] +
                rcma3[rcma3.length - 1] +
                rcma4[rcma4.length - 1]
        );
    }
...
async function obv(data) {
    for (var i = 1, obv = [0]; i < data.length; i++) {
        if (data[i][1] > data[i - 1][1])
            obv.push(obv[obv.length - 1] + data[i][0]);
        if (data[i][1] < data[i - 1][1])
            obv.push(obv[obv.length - 1] - data[i][0]);
...
async function mom(data, length = 10, p) {
    for (var i = length - 1, mom = []; i < data.length; i++) {
        p
            ? mom.push((data[i] / data[i - (length - 1)]) * 100)
            : mom.push(data[i] - data[i - (length - 1)]);
    }
    return mom;
}
async function mom_osc(data, length = 10) {
    length++;
    for (
        var i = length, osc = [], sumh = 0, suml = 0;
        i <= data.length;
        i++, sumh = 0, suml = 0
    ) {
        for (var a = 1; a < length; a++)
            data[i - length + (a - 1)] < data[i - length + a]
                ? (sumh += data[i - length + a])
                : (suml += data[i - length + a]);
...
async function ha(data) {
    var ha = [
        [
            (Number(data[0][0]) + Number(data[0][3])) / 2,
            Number(data[0][1]),
            Number(data[0][2]),
            (Number(data[0][0]) +
                Number(data[0][1]) +
    ...
    for (var i = 1; i < data.length; i++) {
        ha.push([
            (ha[ha.length - 1][0] + ha[ha.length - 1][3]) / 2,
            Math.max(ha[ha.length - 1][0], ha[ha.length - 1][3], data[i][1]),
            Math.min(ha[ha.length - 1][0], ha[ha.length - 1][3], data[i][2]),
            (data[i][0] + data[i][1] + data[i][2] + data[i][3]) / 4,
        ]);
    }
    return ha;
}
async function ren(data, bs = 1) {
    var re = [],
        decimals =
            Math.floor(bs) === bs ? 0 : bs.toString().split('.')[1].length || 0,
        bh =
            (Math.ceil((data[0][0] / bs) * 10 ** decimals) / 10 ** decimals) *
            bs,
        bl = bh - bs;
    for (var i = 1; i < data.length; i++) {
        if (data[i][0] > bh + bs) {
...
async function chaikin_osc(data, ema1 = 3, ema2 = 10) {
    for (var i = 0, cha = [], adl = []; i < data.length; i++) {
        var mfm =
            (data[i][1] - data[i][2] - (data[i][0] - data[i][1])) /
            (data[i][0] - data[i][2]);
        isNaN(mfm) ? adl.push(0) : adl.push(mfm * data[i][3]);
    }
    var ef = await module.exports.ema(adl, ema1),
        es = await module.exports.ema(adl, ema2);
...
async function alligator(
    ...
    for (var i = jaw.length - 1; i >= 7; i--)
        ret.push([jaw[i - (js - 1)], teeth[i - (ts - 1)], lips[i - (ls - 1)]]);
    return ret;
}
async function gator(data, jl = 13, tl = 8, ll = 5, js = 8, ts = 5, ls = 3) {
    ...
    teeth.splice(0, teeth.length - jaw.length);
    lips.splice(0, lips.length - jaw.length);
    for (var i = jaw.length - 1; i >= js - 1; i--)
        ret.push([
            jaw[i - (js - 1)] - teeth[i - (ts - 1)],
            -Math.abs(teeth[i - (ts - 1)] - lips[i - (ls - 1)]),
        ]);
    return ret;
}
...
async function support(d, hl = recent_low(d)) {
    ...
    if (index2 == d.length - 1 || hl.index == d.length - 1)
        return {
            calculate: async (pos) => hl.value,
            slope: 0,
            lowest: hl.value,
            index: hl.index,
        };
    return {
        calculate: async (pos) => pos * lowform + hl.value,
        slope: lowform,
        lowest: hl.value,
        index: hl.index,
    };
...
async function resistance(d, hl) {
    ...
    return {
        calculate: async (pos) => pos * -highform + hl.value,
        slope: highform,
        highest: hl.value,
        index: hl.index,
    };
    ...
    for (let i = len; i <= data.length; i++) {
        var pl = data.slice(i - len, i),
            ...
            v1 =
                0.33 *
                    2 *
                    ((data[i - 1] - mn) / (Math.max.apply(null, pl) - mn) -
                        0.5) +
                0.67 * v1;
        if (v1 > 0.99) v1 = 0.999;
        if (v1 < -0.99) v1 = -0.999;
    ...
    for (let i = atrlen; i <= data.length; i++) {
        ...
        let dev = deviation * atr2,
            highprice = Math.max.apply(
                null,
                pl.slice(pl.length - 1, pl.length).map((x) => x[0])
            ),
            lowprice = Math.min.apply(
                null,
                pl.slice(pl.length - 1, pl.length).map((x) => x[2])
            ),
            highma = await module.exports.sma(
                pl.slice(pl.length - amplitude, pl.length).map((x) => x[0]),
                amplitude
            ),
            lowma = await module.exports.sma(
                pl.slice(pl.length - amplitude, pl.length).map((x) => x[2]),
                amplitude
            );
        if (nexttrend[nexttrend.length - 1] == 1) {
            ...
            if (
                highma[0] < maxlow &&
                pl[pl.length - 1][1] < pl[pl.length - 2][2]
            ) {
                trend.push(1);
                nexttrend.push(0);
                minhigh = pl[pl.length - 2][0];
            ...
            if (
                lowma[0] > minhigh &&
                pl[pl.length - 1][1] < pl[pl.length - 2][0]
            ) {
                trend.push(0);
                nexttrend.push(1);
                maxlow = lowprice;
        ...
        if (trend[trend.length - 1] == 0) {
            if (
                !isNaN(trend[trend.length - 2]) &&
                trend[trend.length - 2] != 0
            ) {
                up.push(
                    isNaN(down[down.length - 2])
                        ? down[down.length - 1]
                        : down[down.length - 2]
                );
            } else {
                up.push(
                    isNaN(up[up.length - 2])
                        ? maxlow
                        : Math.max(up[up.length - 2], maxlow)
                );
            }
            direction = 'long';
            ...
            if (
                ...
                down.push(
                    isNaN(down[down.length - 2])
                        ? minhigh
                        : Math.min(minhigh, down[down.length - 2])
                );
            }
            direction = 'short';
...
async function covariance(data1, data2, length) {
    let out = [],
        x_mean = await module.exports.sma(data1, data1.length),
        y_mean = await module.exports.sma(data2, data2.length);
    for (let z = length; z <= data1.length; z++) {
        let x = data1.slice(z - length, z),
            y = data2.slice(z - length, z),
            x_mean = await module.exports.sma(x, length),
            res = [],
...
const range = (min, max, rng) =>
    Math.floor(
        rng
            ? rng() * (Math.floor(max) - Math.ceil(min)) + Math.ceil(min)
...
const pick = (data, rng) =>
    data[
        Math.floor(
            rng
                ? rng() * Math.floor(data.length)
                : Math.random() * Math.floor(data.length)
        )
    ];
const float = (min, max, rng) =>
    rng ? rng() * (max - min) + min : Math.random() * (max - min) + min;
function prng(seed) {
...
async function zigzag(data, perc = 0.05) {
    ...
    for (let i = 1; i < indexes.length; i++) {
        ...
        for (let x = 1; x <= len; x++)
            final.push(x * delta + indexes[i - 1].value);
    }
    return final;
}
